...
func TestRaftElection(t *testing.T) {
	...
	node1RaftServer, node1srv := raft.StartRaft(node1Lis, node1, node1RaftDirectory, []raft.Node{node2, node3})
	defer node1srv.Stop()
	...
	node2RaftServer, node2srv := raft.StartRaft(node2Lis, node2, node2RaftDirectory, []raft.Node{node1, node3})
	defer node2srv.Stop()
	...
	node3RaftServer, node3srv := raft.StartRaft(node3Lis, node3, node3RaftDirectory, []raft.Node{node1, node2})
	defer node3srv.Stop()
...
func TestRaftPersistentState(t *testing.T) {
	...
	node1RaftDirectory := rafttestutil.CreateRaftDirectory(path.Join(os.TempDir(), "rafttest2", "node1"))
	defer rafttestutil.RemoveRaftDirectory(node1RaftDirectory)
	node1RaftServer, node1srv := raft.StartRaft(node1Lis, node1, node1RaftDirectory, []raft.Node{})
	defer node1srv.Stop()
	defer rafttestutil.StopRaftServer(node1RaftServer)
	...
	node1RebootRaftServer, node1Rebootsrv := raft.StartRaft(node1RebootLis, node1, node1RaftDirectory, []raft.Node{})
...
func TestRaftConfigurationChange(t *testing.T) {
	...
	defer rafttestutil.RemoveRaftDirectory(node4RaftDirectory)
	node4RaftServer, node4srv := raft.StartRaft(node4Lis, node4, node4RaftDirectory, []raft.Node{})
	defer node4srv.Stop()
